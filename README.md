...
人与 M2 之间继续使用 Hermes 现成的消息渠道，省去日常远程管理时登录主机、接管桌面和维护控制隧道的步骤。本机、Herdr 与专用 Gateway 仍需在线，并能访问所选消息平台和模型服务；用户须配置 channel 凭据和允许访问的消息身份。各平台与端到端操作的实测范围见 [开发与验证](#测试)。
...
需要 Python 3.11+；复用专用 Hermes 虚拟环境，额外依赖见 `requirements.txt`。安装和运行不会在启动时自动安装依赖。
...
| 技术 | 用途 |
| --- | --- |
| Python | 插件、生命周期管理与恢复入口 |
| Herdr CLI / RPC | 当前 session 的窗格与 Agent 控制 |
| Hermes Gateway | 已配置的消息渠道与模型接入 |
| hqtui | 本机监控界面 |
| unittest / PTY | 隔离进程、RPC 与终端测试 |
...
仓库未提供项目级 LICENSE；内置 hqtui 的条款见[其许可证](vendor/hqtui/LICENSE)。
