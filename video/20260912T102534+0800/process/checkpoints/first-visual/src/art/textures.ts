...
export function createTextures(lang: Language): Textures {
  ...
  agents.forEach((name, i) => {
    ...
    result[`agent-label${i}`] = label(name, "INDIVIDUAL CONTRIBUTOR");
  });
  result.working = label("WORKING", "MAKING PROGRESS", p.mint);
  result.blocked = label("NEEDS INPUT", "A DECISION IS WAITING", p.amber);
  result.done = label("DONE", "READY FOR REVIEW", p.mint);
  result.herdr = label("Herdr", "MANAGER / M1", p.white);
  result.hermes = label("Hermes", "TRUSTED PROFILE / M2", p.brass);
  result.you = label("YOU", "DIRECTION & JUDGMENT", p.white);
