...
import { candy, type CubieSpec, type Vec3, sceneCubies } from "./cube";
  ...
  return (
    <group position={spec.position} rotation={spec.rotation} scale={spec.scale}>
      <mesh geometry={body} castShadow receiveShadow>
        <meshPhysicalMaterial color={spec.color} roughness={0.3} metalness={0.02}
          clearcoat={0.42} clearcoatRoughness={0.24} envMapIntensity={0.65} />
      </mesh>
      <mesh geometry={sticker} position={[0, 0, 0.499]} receiveShadow>
        <meshPhysicalMaterial color={spec.color} roughness={0.46} clearcoat={0.18}
...
function Rail({ from, to, color = "#bec9b5", width = 0.038 }: {
  from: Vec3; to: Vec3; color?: string; width?: number;
}) {
...
function SessionTray({ position, size }: { position: Vec3; size: [number, number] }) {
  return (
    <group position={position}>
      ...
      <Rail from={[-size[0] / 2, 0.09, size[1] / 2]} to={[size[0] / 2, 0.09, size[1] / 2]} color={hexly.accent} width={0.026} />
    </group>
  );
}

function MessagePhone() {
  const texture = useMemo(() => canvasTexture(640, 1040, (c) => {
...
function Monitor({ seconds }: { seconds: number }) {
  ...
  texture.anisotropy = 8;
  return (
    <group position={[0.1, 0.8, -0.2]} rotation={[0.17, lerp(-0.08, 0.02, between(seconds, 0, 5)), 0]}>
      <mesh geometry={plate} scale={[8.04, 5.10, 2.4]} castShadow receiveShadow>
...
function Environment({ id, seconds }: { id: string; seconds: number }) {
  ...
  useLayoutEffect(() => {
    ...
    camera.position.set(0.5 + Math.sin(seconds * 0.13) * 0.42,
      wide ? 4.0 : 4.35 + Math.sin(seconds * 0.1) * 0.2, wide ? 16.7 : 15.0);
    camera.lookAt(0.1, 0.1, 0);
    camera.updateProjectionMatrix();
  }, [camera, seconds, id]);
  return (
    <>
      <ambientLight intensity={0.55} color="#fffaf1" />
      <hemisphereLight args={["#fffdfa", "#c3d2b8", 1.35]} />
      <directionalLight position={[-5, 9, 7]} intensity={3.0} color="#fff7ec" castShadow
        shadow-mapSize={[2048, 2048]} shadow-camera-left={-12} shadow-camera-right={12}
        shadow-camera-top={10} shadow-camera-bottom={-8} shadow-camera-near={0.5}
        shadow-camera-far={35} shadow-bias={-0.00025} shadow-normalBias={0.025}
        shadow-radius={4} />
      <directionalLight position={[7, 4, -4]} intensity={1.5} color="#eaf3ff" />
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -2.78, 0]} receiveShadow>
        <planeGeometry args={[180, 180]} />
        <shadowMaterial opacity={0.19} color="#536446" toneMapped={false} />
      </mesh>
    </>
  );
}
...
function SetPieces({ id, seconds }: { id: string; seconds: number }) {
  ...
  if (id === "managers") return (
    <>
      ...
      {[-2.7, -0.9, 0.9, 2.7].map((x) => <Rail key={x} from={[x, -1.05, -0.15]} to={[x, -1.75, -0.15]} />)}
...
function Stage({ id, seconds }: { id: string; seconds: number }) {
  const nodes = sceneCubies(id, seconds);
  const center: Vec3 = id === "monitor" ? [2.8, 0.15, 0] : [3.0, -0.03, 0];
  const scale = id === "weekend" ? 1.38 : id === "managers" ? 1.06 : 1.15;
  const turn: Vec3 = id === "weekend"
    ? [-0.03, lerp(-0.38, 0.32, between(seconds, 0, 8)), -0.04]
    : [0, -0.1 + Math.sin(seconds * 0.18) * 0.07, 0];
  return (
    <>
      <Environment id={id} seconds={seconds} />
      <group position={center} scale={scale} rotation={turn}>
        {nodes.map((spec) => <Cubie key={spec.id} spec={spec} />)}
        <SetPieces id={id} seconds={seconds} />
      </group>
      <EffectComposer multisampling={4}>
        <DepthOfField worldFocusDistance={id === "monitor" ? 17 : 15.5}
          worldFocusRange={id === "monitor" ? 10 : 6} bokehScale={1.65}
...
export function World({ id, frame }: { id: string; frame: number }) {
  return (
    <ThreeCanvas width={1920} height={1080} dpr={1} shadows
