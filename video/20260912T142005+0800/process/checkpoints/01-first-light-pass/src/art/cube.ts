...
const faceLabels = [
  ["Codex", "CODING IC"], ["Herdr", "MANAGER"], ["Grok", "CODING IC"],
  ["Pi", "CODING IC"], ["Hermes", "TRUSTED M2"], ["Claude Code", "CODING IC"],
  ["Pane", "MANAGED"], ["Socket", "LOCAL"], ["Caller", "CONTEXT"],
  ["Workflow", "COORDINATE"], [">_", "TERMINAL"], ["Plugin", "EXPLICIT LINK"],
  ["Discord", "CHANNEL"], ["Telegram", "CHANNEL"], ["Slack", "CHANNEL"],
  ["Profile", "CHOSEN"], ["Session", "BOUND OWNER"], ["Local", "CONTROL"],
];

export const grid: Vec3[] = [];
// Front layer first so the key names remain visible at the final camera angle.
for (const z of [1, 0, -1]) for (const y of [1, 0, -1]) for (const x of [-1, 0, 1]) {
  grid.push([x, y, z]);
}
...
export function assembledCube(seconds: number, exploded = true): CubieSpec[] {
  ...
  return grid.map((cell, index) => {
    ...
    if (close < 1) {
      const drift = 1 - close;
      rotation = [drift * Math.sin(index * 3.1) * 0.4,
        drift * Math.cos(index * 1.6) * 0.55, drift * Math.sin(index * 0.8) * 0.3];
    }
    ...
    return {
      id: `cubie-${index}`, label: label[0] ?? "", detail: label[1] ?? "",
      color: Object.values(candy)[(index * 5 + Math.floor(index / 3)) % 6] ?? candy.mint,
      position, rotation, scale: 0.985,
    };
  });
}

const node = (id: string, label: string, color: string, position: Vec3, scale = 1.15,
  detail = "", rotation: Vec3 = [-0.05, 0.12, 0]) =>
...
export function sceneCubies(id: string, seconds: number): CubieSpec[] {
  ...
  if (id === "talent") return [
    node("codex", "Codex", candy.sky, [-2.2, lerp(-3.8, 0.8, f(0.05)) + bob(0), 0.6], 1.32, "CODING IC", [0, 0.2, -0.06]),
    node("grok", "Grok", candy.peach, [-0.45, lerp(-3.8, -0.75, f(0.22)) + bob(1), 1.2], 1.22, "CODING IC", [0.04, -0.15, 0.04]),
    node("pi", "Pi", candy.lemon, [1.05, lerp(-3.8, 1.1, f(0.42)) + bob(2), 0], 1.25, "CODING IC", [0.02, 0.22, 0.08]),
    node("claude", "Claude Code", candy.lilac, [2.55, lerp(-3.8, -0.6, f(0.61)) + bob(3), 0.8], 1.37, "CODING IC", [-0.06, -0.1, -0.06]),
  ...
  if (id === "bind") {
    ...
    return [
      ...nodes.map((cube) => ({ ...cube, scale: 0.985, position: [cube.position[0] + 0.4, cube.position[1] - 0.3, cube.position[2] - 0.6] as Vec3 })),
      node("trusted", "Hermes", candy.oat, [lerp(-2.6, 0.4, f(1.2)), lerp(0.65, -0.3, f(1.2)), lerp(1.8, 0.475, f(1.2))], 0.985, "CHOSEN M2", [0, lerp(-0.55, 0, f(1.2)), 0]),
  ...
  if (id === "managers") return [
    ...
    ...["Codex", "Grok", "Pi", "Claude Code"].map((label, index) =>
      node(label, label, [candy.sky, candy.peach, candy.lemon, candy.lilac][index] ?? candy.sky,
        [-2.7 + index * 1.8, -1.75, 0], 1.06, "CODING IC", [0, 0, 0])),
  ];
  ...
  if (id === "local") return [
    ...assembledCube(0, false).filter((_, index) => index > 8).map((cube) => ({
      ...cube, position: [cube.position[0] + 0.8, cube.position[1] - 0.35, cube.position[2] - 0.6] as Vec3,
    })),
    node("hermes", "Hermes", candy.oat, [0.8, 0.2, 0.8], 1.1, "M2"),
    node("herdr", "Herdr", candy.mint, [0.8, -1, 0.8], 1.1, "LOCAL"),
    node("request", "Message", candy.peach, [-1.8 + f(0.6) * 0.4, 0.25, 0.5], 1.04, "YOUR CHANNEL"),
  ];
  if (id === "monitor") return ["start", "pause", "resume", "stop"].map((label, index) =>
    node(label, label, [candy.mint, candy.lemon, candy.sky, candy.peach][index] ?? candy.mint,
      [-2.9 + index * 1.8, -2.25, 1.1], 0.79, "GATEWAY", [0, 0.04, 0]));
  if (id === "trust") return [
