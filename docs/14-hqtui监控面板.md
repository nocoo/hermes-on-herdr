...
完整离线回归 **153 项通过**，见 [完整输出](evidence/dashboard-unittest.txt)。
...
品牌和新 checkout 使用 `hermes-on-herdr`。已运行的 supervisor 会核验注册目录；迁移时让旧路径通过兼容符号链接解析到新 clone，再更新插件注册路径，可以保留既有进程的所有权检查。原目录可移到备份位置；稳定的 plugin ID、配置目录和控制协议继续识别当前实例。路径兼容与错误目录拒绝已有独立回归测试。
