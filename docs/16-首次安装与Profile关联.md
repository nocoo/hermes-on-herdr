...
**状态：设计方案，0.1.4 尚未实现关联向导。** 已实现的是读取私有配置后的 `bind`，手动步骤见[发布与安装](15-发布与安装.md)。这里定义普通新用户的下一步流程。
...
关联完成后直接进入完整 dashboard；不再提供是否打开监控的询问。Gateway 尚未启动时显示 Start 按钮，复用常驻面板的启动与重试流程。

## 配置差异与已有 Gateway

现有 `profile_preflight` 检查 terminal backend、工作目录、自动加载 shell 配置、工具集和 multiplex 等设置，失败返回带检查项的 `CONFIG_ERROR`。Hermes 自身的插件配置与开关不属于本项目的预检范围。因此自动填 JSON 还不足以让一般用户顺利接入。
...
- 成功以身份和期望平台 READY 为准，失败显示具体原因；dashboard 始终打开，允许一键启动／重试。
