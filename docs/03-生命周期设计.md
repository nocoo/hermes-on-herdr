...
本章是拟实现的算法契约 P。锁、schema、超时、readiness 和退出码命名以 [02](02-系统架构.md) 为准；上游能力见 [H03–H13](09-源码证据索引.md#h03)、[M06–M12](09-源码证据索引.md#m06)。本轮没有运行这些生命周期场景。
...
3. 预检专用 Profile；构造受控环境。每次spawn前再次短拿mutation lock，重读intent/generation，在仍允许运行时完成Popen并登记child后释放；不持锁等待ready。通过 argv 直接创建 `hermes -p herdr-control gateway run --external-supervisor`，同时显式设置 canonical HERMES_HOME。保持与 supervisor 相同的 SID，不 start_new_session、不 nohup、不改成 service start。child stdin为 DEVNULL，stdout/stderr为独立读取管道，不从 hook继承。
...
| Quiesce for maintenance | 保留原desired，写有截止时间的maintenance | 先停child，避免升级／Herdr计划停机触发竞争；到期只恢复原running意图 |
...
非计划退出：supervisor收到HUP/TERM时进入stopping，不再spawn，清理自己child，保留原running意图。若未收到信号，owner socket/pane探测失败超过20秒后执行同样动作；Herdrlivehandoff短暂断连在此之前可恢复。**此watch逻辑也待实现和spike，不是上游现有承诺**。
...
5. 如果用户绕过推荐顺序直接disable/uninstall，周期ensure必须检查registry后不新建；存活supervisor在周期检查发现明确disabled/unlinked时写paused并退出，短暂API失败只进入grace。该补偿待实现，不能说Herdr自己会停服务。
