...
4. `intent.desired=paused` 是持久禁止自动启动 Gateway。自动 ensure 不清 pause、不清 fuse、不修改绑定，但仍恢复 dashboard。
...
后续 owner Herdr Server 启动会执行唯一 startup entry，自动 ensure 专用 dashboard；已有持久 running 意图且未熔断时自动启动 Gateway。已绑定 Profile 的运行配置不兼容时，dashboard 显示 BLOCKED 并定期重试。如果没有初始化或 Profile 路径缺失，返回配置诊断，不尝试默认 Profile。
...
3. 确认 Profile 已存在、`-p` 会解析到该 home。model/provider、平台、运行配置与占用检查由 pane supervisor 在 spawn 前执行；这些检查失败不会阻止 dashboard 出现。预检不加载第三方插件、不解析输出中的 secret、不自动迁移配置。
...
6. paused/fused 禁止启动 Gateway；已有 supervisor 保留 dashboard，无 supervisor 时允许为 dashboard 创建受监管 pane。Gateway 仍按最新意图停止，自动 hook 不清除暂停或熔断。
...
9. 复核 intent revision 仍匹配，查或创建专用 workspace。workspace.create 也有未知结果窗口；pending 的 phase 依次记录 `workspace_requested/workspace_known/pane_requested`。每一步只允许当前 generation 的发起者发送一次非幂等请求。
...
11. 新 supervisor 启动后先拿 lifetime lock，再短拿 mutation lock，校验 pending generation、intent revision 和真实注入 IDs；失败则不启动 child。注册 runtime／R0 控制 socket，释放 mutation lock并启动 dashboard；desired、fuse 与运行预检在 Gateway spawn 前再次核对。
...
2. 在 mutation lock 内确认 pending 有效；登记真实 pane、terminal、workspace、tab，开私有控制 socket和独立 dashboard。paused/fused 时保留面板但不创建 Gateway。R0代表 supervisor可以响应，尚未代表 Gateway ready。
...
4. 保存 child handle和OS fingerprint。读取 stdout/stderr使用持续排空、限量缓冲和轮转，避免满管道阻塞 Gateway；pane 展示完整 dashboard，raw输出只写私有日志。[07](07-安全与运维.md)
...
Stop/Pause、observed clean0或熔断完成 Gateway 清理后，supervisor 保持 lifetime 与 dashboard，显示 PAUSED/FUSED 和 Start 按钮。Start/Resume 复用同一 pane 与 supervisor；暂停尚未完成时，必须等旧 child 和已确认后代退出后才可再 spawn。Herdr 退出、owner 失联超时或插件禁用时，supervisor 才清理面板并退出。

无 child 时的运行预检或 Popen 失败进入 BLOCKED，显示安全的检查项，每 30 秒自动重查，不写永久 fuse。Start 通过新意图 revision 立即重查；Pause 取消启动。异步检查带着开始时的 revision，迟到的旧检查不能授权新 Gateway。dashboard renderer 失败后 5 秒重建，不重启 Gateway。活性轮询仅读取 PID、UID、boot/start 指纹等身份字段，不依赖退出竞态中的 argv 可读性；完整身份检查和信号前复验保留。
...
| Start／Resume | 显式清 pause/fuse，revision+1，running | ensure dashboard；supervisor 预检通过才启动 Gateway，失败则显示 BLOCKED 并自动重试 |
...
直接关闭专用 pane／workspace 视为资源丢失，下一次 ensure 重建 dashboard，并按原意图决定是否启动 Gateway。用户要保持 Gateway 停止应使用 Stop/Pause。dashboard 的 Ctrl+C 通过私有通道持久暂停，面板保持打开；SIGHUP/SIGTERM 因常用于 Herdr 退出不自动写 pause，保留 running 用于恢复。直接执行外部 `hermes gateway stop` 且 supervisor 存活时会观察 clean0 并 latch；若两者同时死亡，插件无法补写从未收到的用户意图。
