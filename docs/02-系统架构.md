...
本章除引用的上游行为外，均为待实现规范 P。固定版本和能力限制见 [01](01-可行性分析.md)，详细算法见 [03](03-生命周期设计.md)。
...
以下是**设计草案**，本仓库当前不放置可安装的根 manifest：
...
实现还应注册 start/resume/stop/restart/status/logs/doctor，全部进入同一个 dispatcher。不要把“初始化 Profile”“创建 pane”“启动 Gateway”拆成多个独立 startup entries。hook argv 的入口相对 plugin root 解析；pane cwd 显式为 plugin root，launcher 再使用绝对模块路径。[H03/H04](09-源码证据索引.md#h03)
...
<canonical HERMES_HOME>/
  ...
  .herdr-gateway-herdr/
    ...
    supervisor.sock.path             仅长路径 fallback 时存在
    logs/events.jsonl                无正文／secret 的结构化事件
    logs/gateway-output.log          私有 child 输出；不能直接公开导出
```

这些 Profile 内文件只会在未来显式初始化时创建，本轮没有触碰。缺 Profile 时只返回诊断，不能偷偷创建默认目录或回退默认 Profile。

`config.json` 字段：`schema=1`、`profile_id`、`profile_home`、`owner_session`、`owner_socket`、`herdr_bin`、`hermes_bin`、`python_bin`、`agent_cwd`、`expected_platforms`、`reconcile_mode`。路径全部绝对化，config 不含模型 API key 或 bot token。runtime-python 是可执行路径的引导副本，必须与 config 一致；非本 UID、可被其他用户改写、格式错误时拒绝执行。
...
```json
{
  ...
  "applied_revision": 12,
  "state": "STARTING",
  "herdr_socket": "/absolute/owner/herdr.sock",
  "profile_home": "/absolute/hermes-root/profiles/herdr-control",
...
binding 还保存 canonical Profile、UID、稳定 owner socket 和首次安装版本；pending 保存 generation、intent revision、create phase、request id、预期 workspace、截止时间。时间同时记录 UTC 用于诊断、monotonic 用于当前进程超时；跨重启重试窗口由 boot identity 与 UTC 保守恢复。
...
本插件 socket 协议1拟定请求：`protocol, id, verb, owner_key, generation, intent_revision`。只允许 `identify/status/stop/restart/quiesce`，不接收任意 argv、路径、signal number 或 shell 文本。响应包含 `ok/code/protocol/id/result`。变更请求必须引用已经持久化的 intent revision，旧 generation／旧 revision 被拒绝；重复 request id 返回原 ACK，不重复发送信号。ACK 仅表示请求被接受，完成需查 status。
