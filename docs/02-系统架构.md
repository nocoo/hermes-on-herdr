...
repo：`nocoo/hermes-on-herdr`；manifest ID：`nocoo.hermes-gateway`；pane entrypoint：`gateway`。根文件名 `herdr-plugin.toml`，min version 设 `0.9.0`。运行时按 [18 · 兼容策略](18-Herdr兼容策略.md) 检查 live ping：稳定 `>=0.9.0,<0.10.0` 且 protocol 22；manifest 的最低版本不能代替该检查。[H01](09-源码证据索引.md#h01)
