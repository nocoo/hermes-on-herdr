...
repo：`nocoo/hermes-on-herdr`；manifest ID：`nocoo.hermes-gateway`；pane entrypoint：`gateway`。根文件名 `herdr-plugin.toml`，min version 设 `0.9.0`。后续更高 Herdr 版本还需 runtime capability check，min version 不是经过测试的上界。[H01](09-源码证据索引.md#h01)
