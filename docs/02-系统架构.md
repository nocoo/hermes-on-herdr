...
flowchart TB
    ...
    S -->|"同 SID 的 foreground child"| G["hermes on herdr<br/>-p herdr-control + external-supervisor"]
    C <-->|"本插件 socket / 原子状态"| S
...
专用 workspace 默认 label `hermes on herdr`，内部一个专用 tab `hermes on herdr`、一个 argv plugin pane。第一次 `workspace.create --no-focus` 会产生初始 shell；先创建受监管 plugin tab，再核验并关闭本次创建且仍空闲的初始 shell，或保留为操作 pane。不能把初始 shell 当 Gateway，也不能为清爽布局关闭含用户工作的 pane。
...
repo：`nocoo/hermes-on-herdr`；manifest ID：`nocoo.hermes-gateway`；pane entrypoint：`gateway`。根文件名 `herdr-plugin.toml`，min version 设 `0.9.0`。运行时按 [18 · 兼容策略](18-Herdr兼容策略.md) 校验 live JSON ping、实际 API 响应与 pane 进程归属；版本号和终端 wire protocol 仅用于诊断，不限制后续 minor/major 版本。[H01](09-源码证据索引.md#h01)

根目录已有[开发 manifest](../herdr-plugin.toml)，以下仅摘录其结构；尚未安装或经过真实 Herdr 验证：

```toml
id = "nocoo.hermes-gateway"
name = "hermes on herdr"
version = "0.1.0"
min_herdr_version = "0.9.0"
platforms = ["macos", "linux"]

[[startup]]
command = ["./bin/hermes-on-herdr", "ensure", "--source", "startup"]

[[events]]
on = "workspace.focused"
command = ["./bin/hermes-on-herdr", "ensure", "--source", "event"]
...
[[panes]]
id = "gateway"
title = "hermes on herdr"
placement = "tab"
command = ["./bin/hermes-on-herdr", "supervise"]

[[actions]]
id = "pause"
title = "hermes on herdr: pause"
command = ["./bin/hermes-on-herdr", "pause"]
```

当前也注册了 monitor/start/resume/stop/restart/status/logs/doctor，全部进入同一个 dispatcher。monitor 恢复并聚焦专用 dashboard，自动 hook 保持 focus=false。只有一个 startup entry。hook argv 的入口相对 plugin root 解析；pane cwd 显式为 plugin root，launcher 再使用绝对模块路径。[H03/H04](09-源码证据索引.md#h03) launcher 使用 `/usr/bin/python3 -IS` 做纯标准库路径校验，再 exec 配置中的 Python 3.11+，保留 venv 路径并启用 `-I -B`。
...
stateDiagram-v2
    ...
    STARTING --> BLOCKED: 运行预检或 Popen 失败
    BLOCKED --> STARTING: 30秒重查或显式 Start
    BLOCKED --> PAUSED: 显式 Pause
    STARTING --> FUSED: child 退出 2 或 78
...
| 启动受阻 / dashboard 恢复 | 预检或 Popen 失败每30秒重查，Start 立即重查；renderer 失败5秒后重开 |
